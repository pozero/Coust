	...
	namespace VK
	{
		namespace Utils
		{
			bool CreatePrimaryCmdBuf(VkCommandPool pool, uint32_t count, VkCommandBuffer* out_buf)
			{
				...
				VK_CHECK(vkAllocateCommandBuffers(g_Device, &info, out_buf));
			...
			bool CreateSemaphores(VkSemaphore* out_Semaphore)
			{
				...
				VK_CHECK(vkCreateSemaphore(g_Device, &info, nullptr, out_Semaphore));
			...
			bool CreateFence(bool signaled, VkFence* out_Fence)
			{
				...
				VK_CHECK(vkCreateFence(g_Device, &info, nullptr, out_Fence));

				return true;
			}

            bool CreateBuffer(const Param_CreateBuffer& info, BufferAlloc& out_BufferAlloc)
            {
                ...
                VK_CHECK(vmaCreateBuffer(g_VmaAlloc, &bufInfo, &allocInfo, &out_BufferAlloc.buffer, &out_BufferAlloc.alloc, nullptr));
			...
			bool CreateImage(const Param_CreateImage& param, ImageAlloc* out_ImageAlloc)
			{
				...
				VK_CHECK(vmaCreateImage(g_VmaAlloc, &info, &allocInfo, &out_ImageAlloc->image, &out_ImageAlloc->alloc, nullptr));
			...
			bool CreateImageView(const Param_CreateImageView& param, VkImageView* out_ImageView)
			{
				...
				VK_CHECK(vkCreateImageView(g_Device, &info, nullptr, out_ImageView));
			...
			bool CreatePipelineLayout(const Param_CreatePipelineLayout& param, VkPipelineLayout* out_PipelineLayout)
			{
				...
				VK_CHECK(vkCreatePipelineLayout(g_Device, &info, nullptr, out_PipelineLayout));
