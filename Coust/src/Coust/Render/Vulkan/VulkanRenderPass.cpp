...
{
	void RenderPassManager::Cleanup(const Context &ctx)
	{
		for (VkRenderPass p : m_CreatedRenderPasses)
		{
			vkDestroyRenderPass(ctx.m_Device, p, nullptr);
		}
	}

	bool RenderPassManager::CreateRenderPass(const Context &ctx, const Swapchain &swapchain, const Param& param, VkRenderPass* out_RenderPass)
	{
		...
		{
			.flags = 0,
			.format = swapchain.m_DepthFormat,
			.samples = ctx.m_MSAASampleCount,
		...
		VK_CHECK(vkCreateRenderPass(ctx.m_Device, &renderPassInfo, nullptr, out_RenderPass));
