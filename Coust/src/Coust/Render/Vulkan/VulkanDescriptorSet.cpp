...
{
	void DesriptorSetManager::Cleanup(const Context &ctx)
	{
		vkDeviceWaitIdle(ctx.m_Device);
		for (VkDescriptorSetLayout l : m_CreatedDescriptorSetLayouts)
		{
			vkDestroyDescriptorSetLayout(ctx.m_Device, l, nullptr);
		}

		for (VkDescriptorPool p : m_CreatedDescriptorPools)
		{
			vkDestroyDescriptorPool(ctx.m_Device, p, nullptr);
		}
	}
	...
	bool DesriptorSetManager::CreateDescriptorSetLayout(const Context &ctx, const Param& param, VkDescriptorSetLayout* out_DescirptorSetLayout)
	{
		uint32_t bind = 0;
		...
		VK_CHECK(vkCreateDescriptorSetLayout(ctx.m_Device, &layoutInfo, nullptr, out_DescirptorSetLayout));

		m_CreatedDescriptorSetLayouts.push_back(*out_DescirptorSetLayout);
		return true;
	}

	bool DesriptorSetManager::CreateDescriptorPool(const Context &ctx, const Param& param, uint32_t setCount, VkDescriptorPool* out_DescriptorPool)
	{
		...
		VK_CHECK(vkCreateDescriptorPool(ctx.m_Device, &descriptorPoolInfo, nullptr, out_DescriptorPool));

		m_CreatedDescriptorPools.push_back(*out_DescriptorPool);
		return true;
	}

	bool DesriptorSetManager::CreateDescriptorSet(const Context &ctx, VkDescriptorPool pool, VkDescriptorSetLayout layout, VkDescriptorSet* out_DescriptorSet)
	{
		...
		VK_CHECK(vkAllocateDescriptorSets(ctx.m_Device, &info, out_DescriptorSet));

		return true;
	}

	void DesriptorSetManager::UpdateDescriptorSet(const Context &ctx, const Param& param, VkDescriptorSet set)
	{
		uint32_t binding = 0;
		...
		vkUpdateDescriptorSets(ctx.m_Device, (uint32_t)writes.size(), writes.data(), 0, nullptr);
	}
}
